...
export const CheckoutPage: React.FC = () => {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      // Mapear los ítems del carrito para el backend
      const mappedItems = items.map((item) => ({
        producto_id: item.producto.id,
        cantidad: item.cantidad,
        personalizacion: item.personalizacion.length > 0 
      ...
      toast.error(error.message || "Error al procesar tu pedido. Por favor, intentá nuevamente.");
    } finally {
