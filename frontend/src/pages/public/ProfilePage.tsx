...
function PedidosSection() {
  ...
  const getDireccionTexto = (id?: number) => {
    ...
    if (!dir) return `Dirección de entrega (ID: ${id})`;
    return `${dir.linea1}${dir.linea2 ? `, ${dir.linea2}` : ""}, ${dir.ciudad}`;
  };
        ...
        {pedidoSeleccionado && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-[#0E0E0E]/95 border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl flex flex-col max-h-[85vh] text-white z-10"
            >
              {/* Decorative glow */}
              <div className="absolute -top-24 -left-24 w-48 h-48 bg-orange-500/5 rounded-full blur-3xl pointer-events-none" />

              {/* Header */}
              <div className="flex items-start justify-between border-b border-white/5 pb-4 mb-6 relative">
                <div className="space-y-1.5">
                  <div className="flex items-center gap-3 flex-wrap">
                    <h3 className="text-xl font-bold tracking-wide" style={{ fontFamily: MONO }}>
                      Pedido #{pedidoSeleccionado.id.toString().padStart(4, "0")}
                    </h3>
                    {(() => {
                      const badge = ESTADO_BADGE[pedidoSeleccionado.estado_codigo] ?? { label: pedidoSeleccionado.estado_codigo, color: "rgba(255,255,255,0.4)", bg: "rgba(255,255,255,0.05)" };
                      return (
                        <span
                          style={{
                  ...
                  <p className="text-xs text-neutral-400" style={{ fontFamily: MONO }}>
                    Realizado el {formatDate(pedidoSeleccionado.created_at)}
                  </p>
                </div>
                <button
                  onClick={() => setPedidoSeleccionado(null)}
              ...
              <div className="flex-1 overflow-y-auto space-y-6 pr-1 -mr-2">
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-orange-500/70 uppercase tracking-wider" style={{ fontFamily: MONO }}>
                    Productos
                  </h4>
                  <div className="space-y-2.5">
                    {pedidoSeleccionado.detalles.map((det, index) => (
                      <div 
                        key={det.id ?? index} 
                        className="flex justify-between items-center py-2.5 px-4 bg-white/[0.02] border border-white/5 rounded-2xl"
                      >
                        <div className="min-w-0 pr-4">
                          <p className="font-semibold text-neutral-200 text-sm truncate">
                            {det.nombre_snapshot}
                          </p>
                          <p className="text-xs text-neutral-500 font-medium mt-0.5" style={{ fontFamily: MONO }}>
                            {det.cantidad} x {formatCurrency(det.precio_snapshot)}
                          </p>
                        </div>
                        <span 
                          className="font-bold text-neutral-300 text-sm flex-shrink-0"
                          style={{ fontFamily: MONO }}
                        >
                          {formatCurrency(det.subtotal_snap)}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-white/[0.01] border border-white/5 rounded-2xl space-y-2">
                    <div className="flex items-center gap-2 text-orange-500/75">
                      <CreditCard size={15} />
                      <span className="text-xs font-bold uppercase tracking-wider" style={{ fontFamily: MONO }}>
                        Medio de Pago
                      </span>
                    </div>
                    <p className="text-sm font-semibold text-neutral-300">
                      {FORMA_PAGO_MAP[pedidoSeleccionado.forma_pago_codigo] ?? pedidoSeleccionado.forma_pago_codigo}
                    </p>
                  </div>
                  ...
                  <div className="p-4 bg-white/[0.01] border border-white/5 rounded-2xl space-y-2">
                    ...
                    <p className="text-sm font-semibold text-neutral-300 leading-snug">
                      {getDireccionTexto(pedidoSeleccionado.direccion_id)}
                    </p>
                  </div>
                </div>
                ...
                {pedidoSeleccionado.notas && (
                  <div className="p-4 bg-orange-500/[0.02] border border-orange-500/10 rounded-2xl space-y-2">
                    <div className="flex items-center gap-2 text-orange-500/75">
                ...
                <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl space-y-3">
                  <div className="flex justify-between items-center text-xs text-neutral-400" style={{ fontFamily: MONO }}>
                    <span>Subtotal</span>
                  ...
                  {pedidoSeleccionado.costo_envio > 0 && (
                    <div className="flex justify-between items-center text-xs text-neutral-400" style={{ fontFamily: MONO }}>
                      ...
                      <span>{formatCurrency(pedidoSeleccionado.costo_envio)}</span>
                    </div>
                  )}

                  {pedidoSeleccionado.descuento > 0 && (
                    <div className="flex justify-between items-center text-xs text-red-400" style={{ fontFamily: MONO }}>
                      <span>Descuento</span>
                      <span>-{formatCurrency(pedidoSeleccionado.descuento)}</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center pt-1">
                    <span className="text-sm font-bold text-neutral-300 uppercase tracking-wider" style={{ fontFamily: MONO }}>
                      Total
                    </span>
                    <span 
                      className="font-bold text-orange-500" 
                      style={{ fontFamily: CORMORANT, fontSize: "1.6rem" }}
                    >
