...
import { MapPin, Plus, Loader2, Home, Briefcase, Info } from "lucide-react";
...
export const AddressSelector: React.FC<AddressSelectorProps> = ({
  ...
  const fetchAddresses = async () => {
    ...
    try {
      ...
      // Pre-seleccionar la principal o la primera disponible
      if (data.length > 0 && selectedId === null) {
        const principal = data.find((d) => d.es_principal);
        if (principal) {
          onSelect(principal.id);
        } else {
          onSelect(data[0].id);
        }
      }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-bold text-white text-base">Dirección de Entrega</h4>
        </div>
        <button
          type="button"
